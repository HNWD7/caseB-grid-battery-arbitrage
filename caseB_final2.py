...
def run_verification(price, pch, pdis, E, label='', verbose=True):
    ...
    # 1. SOC constraint satisfaction: recompute cumulative SOC directly from
    #    decision variables pch/pdis (independent of the sequential loop that
    #    built E) and verify it stays within [0, E_CAP] at every timestep.
    #    For the LP this is independent of run_lp's reconstruction.
    #    For the heuristic this confirms np.clip never activated (guards worked).
    E_cumulative = E0 + np.cumsum(ETA_CH * pch * DT - pdis / ETA_DIS * DT)
    c1 = np.max(np.maximum(
        np.maximum(-E_cumulative, 0),        # lower bound violation
        np.maximum(E_cumulative - E_CAP, 0)  # upper bound violation
    ))
    passed[1] = c1 < TOL
    ...
    if verbose:
        ...
        print(f"{'='*60}")
        print(f"  1  SOC constraint violation: {c1:.2e} kWh      {'PASS' if passed[1] else 'FAIL'}")
...
def run_carbon_pareto(price, carbon):
    ...
    for lc in CARBON_PENALTIES:
        ...
        checks = run_verification(price, pc, pd_, E_, label=f'LP Carbon λ={lc}', verbose=False)
        # Extract Check 1 (SOC constraint violation) for Table 8 reporting
        E_cum = E0 + np.cumsum(ETA_CH * pc * DT - pd_ / ETA_DIS * DT)
        c1 = np.max(np.maximum(np.maximum(-E_cum, 0), np.maximum(E_cum - E_CAP, 0)))
        print(f"  λ_c={lc:4d} GBP/t: profit=£{profit_:,.0f}  emissions={emiss_:.1f} t  "
              f"SOC violation={c1:.2e} kWh  [verified]")
    return results
...
def fig_residual(E_lp, pch_lp, pdis_lp, out_dir):
    T = len(E_lp)
    # Independent check: cumulative SOC recomputed directly from decision
    # variables, minus the sequential-loop SOC returned by run_lp.
    # These are two arithmetically distinct paths to the same quantity;
    # any difference is pure floating-point noise at machine precision.
    E_cumulative = E0 + np.cumsum(ETA_CH * pch_lp * DT - pdis_lp / ETA_DIS * DT)
    resid = E_cumulative - E_lp

    max_abs = np.abs(resid).max()
    exp   = int(np.floor(np.log10(max_abs))) if max_abs > 0 else -13
    scale = 10 ** (-exp)

    fig, ax = plt.subplots(figsize=(7, 2.2))
    ax.plot(range(T), resid * scale, color=COL_LP, lw=0.6, alpha=0.8)
    ax.axhline(0, color='black', lw=0.7, ls='--', alpha=0.5)
    ax.set_xlabel('Hour')
    ax.set_ylabel(f'Residual (×10^{exp} kWh)')
    ax.set_title('SOC Constraint Verification — LP Optimal (should be ≈ 0)', fontweight='bold')
    ax.text(0.99, 0.95,
            f'Max |residual| = {max_abs:.1e} kWh\n(machine precision)',
